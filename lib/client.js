...
function Client(config) {
  ...
  self._queue = Async.queue(function(task, done) {
    ...
    if (config.useOAuth && !defaults.qs.access_token) {
    ...
    defaults.method = task.query.method;
    defaults.uri = task.query.uri;
    if (task.query.qs.formData) {
      defaults.formData = task.query.qs.formData
      delete task.query.qs.formData;
    }

    let options = assign({}, defaults.qs, task.query.qs);
