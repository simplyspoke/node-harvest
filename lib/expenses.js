...
Expenses.prototype.create = function(options, cb) {
	...
	expense += '<notes>' + options.notes + '</notes>';
    if (typeof options.total_cost !== "undefined")
	expense += '<total-cost type="decimal">' + options.total_cost + '</total-cost>';
    if (typeof options.units !== "undefined")
	expense += '<units type="integer">' + options.units + '</units>';
    if (typeof options.project_id !== "undefined")
	expense += '<project-id type="integer">' + options.project_id + '</project-id>';
    if (typeof options.expense_category_id !== "undefined")
	expense += '<expense-category-id type="integer">' + options.expense_category_id + '</expense-category-id>';
    if (typeof options.spent_at !== "undefined")
	expense += '<spent-at type="date">' + options.spent_at + '</spent-at>';
    expense += '</expense>';
