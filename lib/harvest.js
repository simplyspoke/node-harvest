...
function Harvest(config) {
  ...
  this.useOAuth = (helpers.has(config, ['identifier', 'secret', 'redirectUri'])) || (helpers.has(config, 'accessToken'));
  this.useBasic = helpers.has(config, ['email', 'password']);
 ...
 * @param  {[type]}   access_code [description]
 * @param  {Function} cb          [description]
 * @return {[type]}               [description]
 */
