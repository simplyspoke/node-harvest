...
UserAssignment.prototype.assign = function(options, cb) {
	...
	user_assignment += '<id type="integer">' + encode(options.user_id) + '<id>';
...
UserAssignment.prototype.update = function(options, cb) {
    ...
    if (typeof options.user_id !== "undefined")
	user_assignment += '<user-id type="integer">' + encode(options.user_id) + '</user-id>';
    if (typeof options.project_id !== "undefined")
	user_assignment += '<project-id type="integer">' + encode(options.project_id) + '</project-id>';
    if (typeof options.deactivated !== "undefined")
	user_assignment += '<deactivated type="boolean">' + encode(options.deactivated) + '</deactivated>';
    if (typeof options.hourly_rate !== "undefined")
	user_assignment += '<hourly-rate type="decimal">' + encode(options.hourly_rate) + '</hourly-rate>';
    if (typeof options.is_project_manager !== "undefined")
	user_assignment += '<is-project-manager type="boolean">' + encode(options.is_project_manager) + '</is-project-manager>';
