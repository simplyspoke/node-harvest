...
TimeTracking.prototype.create = function(options, cb) {
	...
	time_entry += '<notes>' + encode(options.notes) + '</notes>';
    if (typeof options.hours !== "undefined")
	time_entry += '<hours>' + encode(options.hours) + '</hours>';
    if (typeof options.project_id !== "undefined")
	time_entry += '<project_id type="integer">' + encode(options.project_id) + '</project_id>';
    if (typeof options.task_id !== "undefined")
	time_entry += '<task_id type="integer">' + encode(options.task_id) + '</task_id>';
    if (typeof options.spent_at !== "undefined")
	time_entry += '<spent_at type="date">' + encode(options.spent_at) + '</spent_at>';
    time_entry += '</request>';
