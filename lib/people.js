...
People.prototype.create = function(options, cb) {
	...
	user += '<first-name>' + options.first_name + '</first-name>';
    if (typeof options.last_name !== "undefined")
	user += '<last-name>' + options.last_name + '</last-name>';
    if (typeof options.email !== "undefined")
	user += '<email>' + options.email + '</email>';
    if (typeof options.timezone !== "undefined")
	user += '<timezone>' + options.timezone + '</timezone>';
    if (typeof options.is_admin !== "undefined")
	user += '<is-admin type="boolean">' + options.is_admin + '</is-admin>';
    if (typeof options.telephone !== "undefined")
	user += '<telephone>' + options.telephone + '</telephone>';
    user += '</user>';
